...
ALLOWED_ROOT_DIRS = {".axiom", ".github", "bulk", "data", "gh", "programs", "tests"}
ALLOWED_ROOT_FILES = {
    ".gitignore",
    "CLAUDE.md",
    "README.md",
...
def test_only_ghana_namespace_present() -> None:
    """Ghana is unitary: the only jurisdiction directory is gh/."""
    names = {d.name for d in jurisdiction_dirs()}
    assert names <= {"gh"}, f"unexpected jurisdiction dirs: {names - {'gh'}}"


def test_gh_content_buckets_exist() -> None:
    for marker in ("statutes", "regulations", "policies"):
        assert (ROOT / "gh" / marker).is_dir(), f"missing gh/{marker}"
...
def test_oracle_index_is_gh_scoped() -> None:
    payload = json.loads((ROOT / "data/oracles/oracle-index.json").read_text())
    assert payload["jurisdiction"] == "gh"


def test_source_map_is_gh_scoped() -> None:
    payload = json.loads(
